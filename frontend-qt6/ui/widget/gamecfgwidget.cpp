...
QStringList GameCFGWidget::buildWebCfgLines() const {
  QStringList lines;
  int mapgen = pMapContainer->get_mapgen();

  if (Scripts->currentIndex() > 0) {
    lines << QStringLiteral("script Scripts/Multiplayer/%1.lua")
                 .arg(Scripts
                          ->itemData(Scripts->currentIndex(),
                                     GameStyleModel::ScriptRole)
                          .toString());
  ...
  lines << QStringLiteral("inithealth %1").arg(schemeData(28).toInt());
  lines << QStringLiteral("sd_turns %1").arg(schemeData(29).toInt());
  lines << QStringLiteral("casefreq %1").arg(schemeData(30).toInt());
  lines << QStringLiteral("minestime %1").arg(schemeData(31).toInt() * 1000);
  lines << QStringLiteral("minesnum %1").arg(schemeData(32).toInt());
  lines << QStringLiteral("minedudpct %1").arg(schemeData(33).toInt());
  lines << QStringLiteral("explosives %1").arg(schemeData(34).toInt());
  lines << QStringLiteral("airmines %1").arg(schemeData(35).toInt());
  lines << QStringLiteral("sentries %1").arg(schemeData(36).toInt());
  lines << QStringLiteral("healthprob %1").arg(schemeData(37).toInt());
  lines << QStringLiteral("hcaseamount %1").arg(schemeData(38).toInt());
  lines << QStringLiteral("waterrise %1").arg(schemeData(39).toInt());
  lines << QStringLiteral("healthdec %1").arg(schemeData(40).toInt());
  lines << QStringLiteral("ropepct %1").arg(schemeData(41).toInt());
  lines << QStringLiteral("getawaytime %1").arg(schemeData(42).toInt());
  lines << QStringLiteral("worldedge %1").arg(schemeData(43).toInt());
  lines << QStringLiteral("template_filter %1")
               .arg(pMapContainer->getTemplateFilter());
  lines << QStringLiteral("feature_size %1")
               .arg(pMapContainer->getFeatureSize());
  lines << QStringLiteral("mapgen %1").arg(mapgen);

  if (!schemeData(44).isNull())
    lines << QStringLiteral("scriptparam %1").arg(schemeData(44).toString());
  else
    lines << QStringLiteral("scriptparam");

  switch (mapgen) {
    case MAPGEN_MAZE:
    case MAPGEN_PERLIN:
      lines << QStringLiteral("maze_size %1")
                   .arg(pMapContainer->getMazeSize());
      break;
    case MAPGEN_DRAWN:
      qWarning("Drawn map config not supported in WebAssembly webcfg");
      break;
    default:
      break;
  }
